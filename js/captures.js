...
async function loadNextPuzzle() {
  ...
  if (autoAdvanceCountsEnabled()) {
    if (answerW >= 6) { expandedW = true; renderDigitRow('cap-digits-white', 'w', true); }
    if (answerB >= 6) { expandedB = true; renderDigitRow('cap-digits-black', 'b', true); }
  }
...
async function showSummary() {
  ...
  if (count > 0) {
    ...
    const { accMet, timeMet, hasGoal } = await checkGoals('captures', count, totalCorrect, totalMisses, totalSeconds);
