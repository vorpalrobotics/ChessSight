...
export async function startUnder() {
  ...
  stopTimer();
  await runWalkthrough('under', buildWalkthrough('under'));
  seconds = 0;
...
async function showSummary() {
  ...
  await updateSummaryGoals('under', count);
  if (count > 0) {
    ...
    const { accMet, timeMet, hasGoal } = await checkGoals('under', count, totalCorrect, totalMisses, totalSeconds);
...
function setStatus(msg) {
  document.getElementById('under-status').textContent = msg;
}

function startTimer() {
  timerInterval = setInterval(() => {
    seconds++;
    document.getElementById('under-timer').textContent = formatTime(seconds);
  }, 1000);
}
