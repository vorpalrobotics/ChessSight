...
export async function startLoose() {
  ...
  stopTimer();
  await runWalkthrough('loose', buildWalkthrough('loose'));
  seconds = 0;
...
function updateProgress() {
  const fill  = document.getElementById('loose-progress-fill');
  const label = document.getElementById('loose-progress-label');
...
async function showSummary() {
  ...
  if (count > 0) {
    ...
    const { accMet, timeMet, hasGoal } = await checkGoals('loose', count, totalCorrect, totalMisses, totalSeconds);
    if (accMet || timeMet) await showGoalCelebration(accMet, timeMet, accuracy, totalSeconds / count);
    const isPB = await checkAndUpdatePB('loose', count, totalCorrect, totalMisses, totalSeconds);
